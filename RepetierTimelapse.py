...
class RepetierTimelapse(Script):
    def __init__(self):
        super().__init__()

    def getSettingDataString(self):
        return """{
            "name": "Repetier timelapse",
            "key": "RepetierTimelapse",
            "metadata": {},
            "version": 2,
            "settings":
            {
                ...
                "move_gcode":
                {
                    "label": "GCODE to move to the photo position.",
                    "description": "GCODE to move to the photo position.",
                    "type": "str",
    ...
    def execute(self, data):
        ...
        move_gcode = self.getSettingValueByKey("move_gcode")
        ...
        for layerIndex, layer in enumerate(data):
            ...
            for line in lines:
                if ";LAYER:" in line:
                    ...
                    if activate_plugin:
                        gcode_to_append += ";RepetierTimelapse Begin\n"
                        ...
                        # Move print head to photo position
                        gcode_to_append += move_gcode + ";GCODE for the first position(display position)\n"
                        gcode_to_append += second_gcode + ";GCODE for the second position(trigger position)\n"
                        gcode_to_append += self.putValue(M = 400) + ";Wait for moves to finish\n"
                        gcode_to_append += ";@syncMotion;Wait for queue to clear\n"

                        # Trigger Photo Command
                        ...
                        # Move print head back to original position
                        ...
                        # Move print head back to original z position
                        gcode_to_append += self.putValue(G = 91) + ";Switch to relative positioning\n"
                        gcode_to_append += self.putValue(G = 0, Z = -1) + ";Restore Z axis position\n"
                        gcode_to_append += self.putValue(G = 90) + ";Switch back to absolute positioning\n"

                        gcode_to_append += ";RepetierTimelapse End\n"
